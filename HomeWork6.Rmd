...
Western_Europe %>%
  ggplot(aes(x = Year, 
    ...
    geom_point(shape=1, color="black", fill="#69b3a2", size=6) +
    ggtitle("Homicide rate in Western Europe from 1300-2000")
```
...
```{r}
Graph <- ggplot(data = Western_Europe) + 
  geom_point(mapping = aes(x = Year, 
                           y = homicide, 
                           color = Country)) +
  labs(x = "Year",
...
Graph + facet_wrap(~ Country, ncol = 2, nrow = 3)
```
